...
(deftest generic-token-round-trips
  ...
  (testing "incoming: an unregistered server tag arrives as a token"
    ;; a raw server emission — the client has no handler for flowdom/db
    (let [wire "[\"~#flowdom/db\",[\"^ \",\"~:basis-t\",1010]]"]
      (is (= (transit/token "flowdom/db" {:basis-t 1010})
             (transit/read wire)))))
  (testing "tokens are plain data: value equality, readable rep"
    (is (= (transit/token "t" {:a 1}) (transit/token "t" {:a 1})))
    (let [r (transit/read "[\"~#flowdom/db\",[\"^ \",\"~:basis-t\",1010]]")]
      (is (transit/token? r))
      (is (= "flowdom/db" (transit/token-tag r)))
