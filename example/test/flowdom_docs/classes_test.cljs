(ns flowdom-docs.classes-test
  "Guards lib/flowdom-docs' Tailwind class manifest
  (resources/flowdom/docs/classes.txt), which consumers outside this
...
;; every .cljs source in lib/flowdom-docs — extend when the lib grows
...
(def ^:private sources
  [(rc/inline "flowdom/docs.cljs")
   (rc/inline "flowdom/docs/ui.cljs")])

(def ^:private manifest (rc/inline "flowdom/docs/classes.txt"))
  ...
  (let [used    (apply set/union (map tokens sources))
    ...
    (is (empty? missing)
        (str "tokens in lib source but not in the manifest — run "
             "`clojure -M:gen` in lib/flowdom-docs: " (pr-str (sort missing))))
    (is (empty? stale)
