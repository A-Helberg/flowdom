(ns flowrpc.db-transit-test
  "The db-as-value serialization boundary, without Datomic: a fake db
  type stands in for datomic.db.Db. Write side: value → #flowdom/db
  {:basis-t t}. Read side: a resolver turns the token back into an
  actual value; without one, the tag reads as a generic Token record.
...
(deftest db-value-round-trips-through-the-ref
  (testing "value → wire → value: the fn on the other side receives a db"
    (let [wire (transit/write {:fn-name 'api.notes/all-notes<
      ...
      (is (= ["one"] (:notes (first args))) "resolved, queryable — not a token")))
  (testing "the wire form is the token, not the data"
  ...
  (testing "without a resolver, the tag reads as a generic Ref"
    (let [wire (transit/write (get history 7) {:handlers db-out})]
      (is (= (transit/token transit/db-tag {:basis-t 7}) (transit/read wire))))))
