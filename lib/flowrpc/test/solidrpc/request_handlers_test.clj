(ns flowrpc.request-handlers-test
  ...
  it — flowrpc never learns the session mechanism."
...
(deftest query-read-handler-closes-over-the-request
  ...
  (let [req  (assoc (query-req 'flowrpc.request-handlers-test/whoami (token "abc"))
...
(deftest query-write-handlers-ride-the-stream
  ...
  (let [resp (server/handle-query
              {:query-params {"q" (transit/write {:fn-name 'flowrpc.request-handlers-test/boxed-stream
                                                  :args []})}}
...
(deftest rejecting-read-handler-maps-to-its-status
  (registry/register! #'whoami)
  (let [resp (server/handle-query
              (query-req 'flowrpc.request-handlers-test/whoami (token "bad"))
              {:read-handlers
               {"test/user" (fn [_] (throw (ex-info "no session" {:flowrpc/status 401})))}})]
    (is (= 401 (:status resp))
...
(deftest command-read-and-write-handlers
  ...
  (testing "decode side"
    (let [resp (server/handle-command
                (command-req 'flowrpc.request-handlers-test/whoami (token "abc"))
  ...
  (testing "encode side"
    (let [resp (server/handle-command
                (command-req 'flowrpc.request-handlers-test/boxed-command)
                {:write-handlers {Box {:tag "test/box" :rep :v}}})
...
(deftest command-rejection-maps-to-its-status
  (registry/register! #'whoami)
  (let [resp (server/handle-command
              (command-req 'flowrpc.request-handlers-test/whoami (token "bad"))
              {:read-handlers
               {"test/user" (fn [_] (throw (ex-info "expired" {:flowrpc/status 401})))}})]
    (is (= 401 (:status resp)))
