...
(defn query
  "Returns a Missionary flow backed by an SSE connection.
   ...
   enclosing :fallback. A followed ref holding `unresolved` (see below)
   keeps the query silent entirely.

   A followed ref CHANGING is silent by default: readers keep the
   stale answer until the new connection's first value. Include the
   `loading-visible` sentinel among the args to surface each refetch
   as loading instead — see its docstring."
  [fn-id & args]
  (assert (symbol? fn-id) "query requires a symbol")
  (stream/follow-args (vec args) #(stream/sse-flow fn-id %)))

(def unresolved
  "Initial value for a selection ref that queries follow: the value
  isn't known YET (distinct from nil = resolved to nothing). While a
  followed ref holds this, the query emits nothing — loading holds,
  and the query is never asked the wrong (initial) question. Use it
  when a query's argument derives from another query's answer."
  stream/unresolved)

(def loading-visible
  "Sentinel: include it anywhere in a query's args to surface
  refetches as loading. Each (re)connect emits :flowdom.rx/pending —
  flowdom's pending protocol — before the answer, so readers re-enter
  loading on a followed ref's change instead of holding the stale
  answer. Off by default: omit for stale-while-refetch."
  stream/loading-visible)

(def loading-value
  "(loading-value x) anywhere in a query's args: the placeholder is
  emitted before the query's FIRST answer, wrapped as
  [:flowdom.rx/loading x] — flowdom's loading-with-value form. `?`
  hands the wrapper through (no suspense), `flowdom.rx/value`
  unwraps it for rendering, `loading?<` reads true while it is
  current — the placeholder stays distinguishable from answers.
  First answer only: refetches do not re-emit it. Composes with
  `loading-visible` (placeholder initially, loading marker on
  refetches)."
