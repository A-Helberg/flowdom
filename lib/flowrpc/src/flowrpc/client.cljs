...
(def loading-value
  "(loading-value x) anywhere in a query's args: the placeholder `x`
  is emitted before the query's FIRST answer — the mount renders `x`
  immediately instead of pending. First answer only: refetches do
  not re-emit it. Composes with `loading-visible` (placeholder
  initially, loading marker on refetches). Readers can't tell `x`
  from an answer, so pass one only when an immediate value is the
  right semantics."
