(ns flowrpc.transit
  ...
  — in practice, the opts you hand flowrpc.server's handlers at the
  ...
  (token \"flowdom/db\" {:basis-t 1010}) — value equality, round-trips
  ...
  the server writes it as #flowdom/db {:basis-t t} (a type-dispatched
...
#?(:cljs
   (extend-type ty/TaggedValue
     ...
     (-pr-writer [this writer _opts]
       (-write writer (str "#flowrpc/token [" (.-tag this) " "
                           (pr-str (.-rep this)) "]")))))
