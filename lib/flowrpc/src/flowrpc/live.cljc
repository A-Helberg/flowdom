(ns flowrpc.live
  ...
  are cheap); when a value crosses the wire, flowrpc.transit
  serializes it as a token (#flowdom/db {:basis-t t}) and deserializes it
  ...
  flowrpc.server streams it over SSE; or hold it directly in-process
...
(defn live
  ...
  (when (nil? reports<)
    (throw (ex-info "flowrpc.live: a reports< flow is required"
                    {:reports< reports<})))
