...
(defn loading-value
  ...
  query's args and [:flowdom.rx/loading x] — flowdom's
  loading-with-value form — is emitted before the query's FIRST
  answer. The wrapper travels the value channel: readers get it from
  ? as-is (check flowdom.rx/loading?, unwrap with flowdom.rx/value),
  loading?< reads true while it is current, and answers arrive bare —
  the placeholder stays distinguishable. First answer only: a refetch
  (followed ref change) does not re-emit it — readers hold the stale
  answer as usual, or see the pending marker if `loading-visible` is
  also passed (the two compose: placeholder initially, loading on
  refetches; the first connect emits the placeholder, which preempts
  the marker). `unresolved` refs still emit nothing at all."
  [v]
  (->LoadingValue v))
...
(defn- wrap-loading
  "flowdom's loading-with-value form, as plain data — flowrpc does
  not depend on flowdom, same deal as the pending marker."
  [x]
  [:flowdom.rx/loading x])
...
(defn follow-args
  ...
  (let [visible? (boolean (some #{loading-visible} args))
    ...
    (if-not (some watchable? args)
      (cond
        lv       (prepend (wrap-loading @lv) (make-flow args))
           ...
           (let [vs (m/?< (m/eduction (dedupe) args<))]
             (if (some #(= unresolved %) vs)
               ...
               (let [first? (not @connected*)
                     ...
                     inner  (cond
                              (and first? lv) (prepend (wrap-loading @lv) (make-flow vs))
