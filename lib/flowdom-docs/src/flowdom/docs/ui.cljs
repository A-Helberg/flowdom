...
(defn input
  "[ui/input {:value a :on-input f :placeholder s :name s :type s}]
  — a text input. `:value` may be an atom (live), an rx, or a plain
  value; `:name`/`:type` pass through for uncontrolled forms."
  [{:keys [value on-input placeholder name type]}]
  (let [value (if (and (some? value)
                       (not (frx/rx? value))
    ...
    [:input (cond-> {:class "border border-gray-300 rounded-md px-2 py-1 text-sm w-56 focus:outline-2 focus:outline-blue-500"}
              ...
              (some? on-input)    (assoc :on-input on-input)
