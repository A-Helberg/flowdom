...
A docs/tutorial shell for [flowdom](../flowdom) apps: sidebar
...
2. an `@source` covering this lib's classes. Pick one:
   ...
   **Monorepo / `:local/root` dep** — scan the source directly:
   ...
   ```css
   @source "../path/to/flowdom-docs/src";
   ```

   **Git or jar dep** — the lib ships a class manifest on the
   classpath at `flowdom/docs/classes.txt`. Copy it into your repo
   once per lib upgrade:

   ```sh
   clojure -M -e '(spit "flowdom-docs.classes.txt" (slurp (clojure.java.io/resource "flowdom/docs/classes.txt")))'
   ```

   ```css
   @source "./flowdom-docs.classes.txt";
   ```
